...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombre) || empty($precio) || empty($categoria)) {
            ...
            if (!in_array($extension, $permitidas)) {
                ...
                if (move_uploaded_file($_FILES['imagen']['tmp_name'], $destino)) {
                    if (!empty($imagenActual)) {
                        @unlink('../uploads/platillos/' . $imagenActual);
                    }
        ...
        if (empty($mensaje)) {
            $sql = "UPDATE platillos SET nombre=?, precio=?, idCategoria=?, idSubcategoria=?, disponible=?, imagen=? WHERE idPlatillo=?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, 'sdiissi', $nombre, $precio, $categoria, $subcategoria, $disponible, $imagen, $id);
...
$platillo = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM platillos WHERE idPlatillo = $id"));
...
<head>
    ...
    <title>Ukiyo | Editar Platillo</title>
...
<div class="inner-content">
    <div class="section-header">
        ...
        <span class="section-title">Editar platillo</span>
    </div>
    ...
    <div class="card-ukiyo">
        <div class="card-header-ukiyo"><i class="bi bi-pencil"></i> Editar platillo</div>
        <div class="card-body-ukiyo">
            <form method="POST" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?php echo $id; ?>">
                <input type="hidden" name="imagen_actual" value="<?php echo $platillo['imagen']; ?>">
                <div class="row g-3">
                    <div class="col-md-5">
                        <label class="form-label">Nombre del platillo</label>
                        <input type="text" name="nombre" class="form-control" value="<?php echo $platillo['nombre']; ?>" required>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Precio</label>
                        <div class="input-group">
                            <span class="input-group-text">$</span>
                            <input type="number" name="precio" class="form-control" value="<?php echo $platillo['precio']; ?>" step="0.01" required>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Disponible</label>
                        <div class="form-check form-switch mt-2">
                            <input class="form-check-input" type="checkbox" name="disponible" <?php echo $platillo['disponible'] ? 'checked' : ''; ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="categoria" class="form-select" id="categoria" onchange="filtrarSubcategorias()" required>
                            ...
                            <option value="<?php echo $cat['idCategoria']; ?>" <?php echo $cat['idCategoria'] == $platillo['idCategoria'] ? 'selected' : ''; ?>>
                                <?php echo $cat['nombre']; ?>
                            </option>
                            <?php endwhile; ?>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="subcategoria" class="form-select" id="subcategoria">
                            ...
                            <option value="<?php echo $sub['idSubcategoria']; ?>" data-categoria="<?php echo $sub['idCategoria']; ?>" <?php echo $sub['idSubcategoria'] == $platillo['idSubcategoria'] ? 'selected' : ''; ?>>
                                <?php echo $sub['nombre']; ?>
                            </option>
                            <?php endwhile; ?>
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="mb-2">
                            <img src="../uploads/platillos/<?php echo $platillo['imagen']; ?>" alt="Imagen actual" style="width:100px; height:100px; object-fit:cover; border-radius:8px; border:2px solid #eee;">
                            <span class="ms-2" style="font-size:12px; color:#999;">Imagen actual</span>
                        </div>
                        <?php endif; ?>
                        <input type="file" name="imagen" class="form-control" accept=".jpg,.jpeg,.png,.webp" onchange="previsualizarImagen(this)">
                        <div class="form-text">Opcional. Si subes una nueva imagen reemplazara la actual.</div>
                    </div>
